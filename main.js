...
function showHome(push=true){cleanup();if(push)history.pushState({},'',location.pathname);app.innerHTML=`<section class="home-hero"><div><span class="eyebrow">GG DIGITAL • DOCUMENTOS</span><h1>Escolha o documento que deseja preencher.</h1><p>O Assinador GG usa o PDF original como modelo, insere os dados nos campos corretos e gera o arquivo pronto no seu dispositivo.</p></div><aside class="hero-note"><strong>Privacidade por padrão</strong><p>Os dados e a assinatura manuscrita são processados no navegador. Este MVP não cria cadastro nem salva as informações em banco.</p></aside></section><section><div class="toolbar"><div><span class="section-kicker">Biblioteca</span><h2>Modelos disponíveis</h2></div><label class="search"><input id="modelSearch" type="search" placeholder="Buscar documento..." aria-label="Buscar documento"></label></div><div id="modelGrid" class="model-grid">${MODELS.map(card).join('')}</div></section>`;bindCards();app.querySelector('#modelSearch').addEventListener('input',e=>filter(e.target.value));}
