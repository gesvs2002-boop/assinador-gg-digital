...
export async function createJiefPdf(data) {
  ...
  entered.forEach(model=>{
    ...
    model.entries.forEach((entry,index)=>{
      ...
      page.drawText(`${entry.ra||entry.ria?`RA ${entry.ra||entry.ria}  •  `:''}${athleteCode(entry.athlete_id)}`,{x:78,y:rowY-25,size:7,font:fonts.bold,color:PDF.rgb(.58,.29,.12),maxWidth:292});
